...
#read data
pol_dat<-read.csv("Nigeria_NC.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2021-06-08','2022-07-10')) )
...
#Incidence
Nigeria_NC_Incidence$Date_Onset<-as.Date(Nigeria_NC_Incidence$Date_Onset)
library(ggplot2)
library(incidence)
dat<-Nigeria_NC_Incidence$Date_Onset
i<-incidence(dat)
plot(i)
...
#Campaign Plot
campaigns<-read.csv("Nigeria_NC_Campaigns.csv")
...
#EpiEstim Plots
# add on 0s for last 3 months if last case was more than 6 months ago
Nigeria_EpiEstim_Data<-read.csv("Nigeria_NC_EpiEstim.csv")
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Nigeria_NC Incidence Plot",
                           "Nigeria_NC Campaign Plot",
                           "Nigeria_NC Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Nigeria_NC Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Nigeria_NC Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Nigeria_NC Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
