...
#read data
pol_dat<-read.csv("Nigeria_S.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2021-05-21','2021-11-26')) )
...
#Incidence
Nigeria_S_Incidence$Date_Onset<-as.Date(Nigeria_S_Incidence$Date_Onset)
library(ggplot2)
library(incidence)
dat<-Nigeria_S_Incidence$Date_Onset
i<-incidence(dat)
plot(i)
...
#Campaign Plot
campaigns<-read.csv("Nigeria_S_Campaigns.csv")
...
#EpiEstim Plots
# add on 0s for last 3 months if last case was more than 6 months ago
Nigeria_EpiEstim_Data<-read.csv("Nigeria_S_EpiEstim.csv")
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Nigeria_S Incidence Plot",
                           "Nigeria_S Campaign Plot",
                           "Nigeria_S Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Nigeria_S Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Nigeria_S Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Nigeria_S Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
