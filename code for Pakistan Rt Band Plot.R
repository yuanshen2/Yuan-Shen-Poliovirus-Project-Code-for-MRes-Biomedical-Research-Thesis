...
#Data for Activity Start Dates
X2019Pakistan_campaigns <- read_csv("~/EpiFliter/2019Pakistan campaigns.csv")
library(lubridate)
X2019Pakistan_campaigns$`Activity Start Date`<-dmy(X2019Pakistan_campaigns$`Activity Start Date`)
X2019Pakistan_campaigns$`Activity End Date`<-dmy(X2019Pakistan_campaigns$`Activity End Date`)
X2019Pakistan_campaigns$`Activity Start Date`<-as.Date(X2019Pakistan_campaigns$`Activity Start Date`)
X2019Pakistan_campaigns$`Activity End Date`<-as.Date(X2019Pakistan_campaigns$`Activity End Date`)
library(tidyverse)
library(dplyr)
Pakistan_campaigns<-mutate(X2019Pakistan_campaigns,
                           mid_dates=X2019Pakistan_campaigns$`Activity Start Date`+
                             (X2019Pakistan_campaigns$`Activity End Date`-X2019Pakistan_campaigns$`Activity Start Date`)/2)

#Add 28 days to start dates and mid dates
start_dates<-Pakistan_campaigns$`Activity Start Date`+28
mid_dates<-Pakistan_campaigns$mid_dates+28
df_rect<-data.frame(start=start_dates, 
                      mid=mid_dates)

combine<-ggplot() +
  geom_rect(data = df_rect, aes(xmin = start, xmax = mid,
                                ymin = -Inf, ymax = Inf),
            fill = "slateblue", alpha = 0.2) +
  geom_line(data = p$data, aes(x = dates, y = values)) +
  geom_hline(yintercept = sum(pR0[1:20]), color = "blue", linetype = "solid") +
...
write.csv(Pakistan_campaigns,
          "C:\\Users\\25673\\Documents\\EpiFliter\\Pakistan_2019campaigns_with_mid.csv",
