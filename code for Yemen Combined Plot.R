...
#read data
pol_dat<-read.csv("~/EpiFliter/YEMEN-YEM-TAI-1.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2021-07-06','2022-10-12')) )
...
#Incidence Plot
YEMEN_YEM_TAI_1$`Date Onset`<-as.Date(YEMEN_YEM_TAI_1$`Date Onset`)
library(ggplot2)
library(incidence)
dat<-YEMEN_YEM_TAI_1$`Date Onset`
i<-incidence(dat)
plot(i)
...
#Campaign Plot
Yemen_Campaign_Data$`Campaign Dates`<-as.Date(Yemen_Campaign_Data$`Campaign Dates`)

library(ggplot2)
paklabs <- ggplot(Yemen_Campaign_Data,aes(x=`Campaign Dates`,y=`Targeted Population`))+
  geom_col()+
  scale_x_date(date_breaks = "3 months",
               limits =as.Date(c('2021-07-06','2022-10-12')) )
paklabs
...
#EpiEstim Plot
# add on 0s for last 3 months if last case was more than 6 months ago
YemenEpiEstimData<-read.csv("~/EpiFliter/YemenEpiEstimData.csv")
YemenEpiEstimData$dates<-as.Date(YemenEpiEstimData$dates)
last_case <-max(YemenEpiEstimData$dates)
today <- as.Date("2022-12-13")
if (last_case<today-6*30){
  Ipol <-c(rep(0,90))
...
T<-nrow(YemenEpiEstimData)
t_start<-seq(2,T-29)
t_end<-t_start+29
res_weekly<-estimate_R(YemenEpiEstimData,
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Yemen Incidence Plot",
                           "Yemen Campaign Plot",
                           "Yemen Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Yemen Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Yemen Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Yemen Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
