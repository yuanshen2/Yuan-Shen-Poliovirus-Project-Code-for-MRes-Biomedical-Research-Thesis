...
#A Afghanistan Incidence Plot
Afghanistan_Incidence_Data <- read_csv("~/EpiFliter/Afghanistan Incidence Data.csv")
...
library(ggplot2)
pak <- ggplot(df_complete,aes(x=`mid`,y=`total_targeted`))+
  geom_col()+
  scale_x_date(date_breaks = "3 months",
               limits =as.Date(c('2020-01-29','2021-12-15')) )
pak
...
#D Afghanistan EpiEstim Plot
# add on 0s for last 3 months if last case was more than 6 months ago
Afghanistan_EpiEstim_Data<-read_csv("~/EpiFliter/Afghanistan EpiEstim Data.csv")
