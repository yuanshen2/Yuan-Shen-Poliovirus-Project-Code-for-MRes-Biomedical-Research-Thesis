...
#read data
pol_dat<-read.csv("~/EpiFliter/Nigeria.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2020-07-12','2022-10-10')) )
...
#Incidence
Nigeria$`Date Onset`<-as.Date(Nigeria$`Date Onset`)
library(ggplot2)
library(incidence)
dat<-Nigeria$`Date Onset`
i<-incidence(dat)
plot(i)
...
#Campaign Plot
campaigns<-read.csv("~/EpiFliter/Nigeria campaigns.csv")
...
#EpiEstim Plots
# add on 0s for last 3 months if last case was more than 6 months ago
Nigeria_EpiEstim_Data<-read.csv("Nigeria EpiEstim Data.csv")
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Nigeria Incidence Plot",
                           "Nigeria Campaign Plot",
                           "Nigeria Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Nigeria Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Nigeria Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Nigeria Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
