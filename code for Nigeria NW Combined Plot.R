...
#read data
pol_dat<-read.csv("Nigeria_NW.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2020-07-12','2022-08-13')) )
...
#Incidence
Nigeria_NW_Incidence$Date_Onset<-as.Date(Nigeria_NW_Incidence$Date_Onset)
library(ggplot2)
library(incidence)
dat<-Nigeria_NW_Incidence$Date_Onset
i<-incidence(dat)
plot(i)
...
#Campaign Plot
campaigns<-read.csv("Nigeria_NW_Campaigns.csv")
campaigns$Sub.Activity.Start.Date <- as.Date(campaigns$Sub.Activity.Start.Date)
campaigns$Sub.Activity.End.Date<-as.Date(campaigns$Sub.Activity.End.Date)
campaigns$mid<-campaigns$Sub.Activity.Start.Date+(campaigns$Sub.Activity.End.Date-campaigns$Sub.Activity.Start.Date)/2
...
library(ggplot2)
pak <- ggplot(daily_campaigns,aes(x=`mid`,y=`total_targeted`))+
  geom_col()+
  scale_x_date(date_breaks = "3 months",
               limits =as.Date(c('2020-07-12','2022-08-13')) )
pak

print(pak+labs(y="Targeted Population",x="Activity Dates"))

paklabs<-pak+labs(y="Targeted Population",x="Activity Dates")
print(paklabs)

#EpiEstim Plots
# add on 0s for last 3 months if last case was more than 6 months ago
Nigeria_EpiEstim_Data<-read.csv("Nigeria_NW_EpiEstim.csv")
Nigeria_EpiEstim_Data$dates<-as.Date(Nigeria_EpiEstim_Data$dates)
last_case <-max(Nigeria_EpiEstim_Data$dates)
today <- as.Date("2022-11-25")
if (last_case<today-6*30){
  ...
  Nigeria_EpiEstim_Data <- rbind(Nigeria_EpiEstim_Data,temp)
}
library(EpiEstim)
library(ggplot2)
T<-nrow(Nigeria_EpiEstim_Data)
t_start<-seq(2,T-29)
t_end<-t_start+29
res_weekly<-estimate_R(Nigeria_EpiEstim_Data,
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Nigeria_NW Incidence Plot",
                           "Nigeria_NW Campaign Plot",
                           "Nigeria_NW Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Nigeria_NW Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Nigeria_NW Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Nigeria_NW Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
