...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2020-01-29','2021-12-15')) )
...
#insert fit
tset = 1:length(Rhat) # use dates instead
...
pl4<-pl4+scale_y_log10()+coord_cartesian(ylim=c(0.5,10))#Missing argument to function call
pl4

#incidence plot
Afghanistan_Incidence_Data$`Date Onset`<-as.Date(Afghanistan_Incidence_Data$`Date Onset`)
library(ggplot2)
library(incidence)
dat<-Afghanistan_Incidence_Data$`Date Onset`
i<-incidence(dat)
plot(i)
...
campaigns<-read.csv("~/EpiFliter/Afghanistan campaigns.csv")
...
#EpiEstim Plot
# add on 0s for last 3 months if last case was more than 6 months ago
Afghanistan_EpiEstim_Data<-read.csv("Afghanistan EpiEstim Data.csv")
Afghanistan_EpiEstim_Data$dates<-as.Date(Afghanistan_EpiEstim_Data$dates)
last_case <-max(Afghanistan_EpiEstim_Data$dates)
today <- as.Date("2022-11-25")
if (last_case<today-6*30){
  ...
  Afghanistan_EpiEstim_Data<- rbind(Afghanistan_EpiEstim_Data,temp)
}
library(EpiEstim)
library(ggplot2)
T<-nrow(Afghanistan_EpiEstim_Data)
t_start<-seq(2,T-29)
t_end<-t_start+29
res_weekly<-estimate_R(Afghanistan_EpiEstim_Data,
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("Afghanistan Incidence Plot",
                           "Afghanistan Campaign Plot",
                           "Afghanistan Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "Afghanistan Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "Afghanistan Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "Afghanistan Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
