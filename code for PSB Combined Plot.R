...
#read data
pol_dat<-read.csv("Pakistan_PSB.csv",stringsAsFactors = F)
#format dates
...
pl2<-pl2+scale_x_date(date_breaks = "3 months",
                      limits =as.Date(c('2019-11-08','2021-11-20')) )
...
#Incidence Plot
Pakistan_PSB_Incidence$Date_Onset<-as.Date(Pakistan_PSB_Incidence$Date_Onset)
library(ggplot2)
library(incidence)
dat<-Pakistan_PSB_Incidence$Date_Onset
i<-incidence(dat)
plot(i)
...
#Campaign Plot
campaigns<-read.csv("R files/PSB Campaigns.csv")
...
#EpiEstim Plot
# add on 0s for last 3 months if last case was more than 6 months ago
Pakistan_EpiEstim_Data<-read.csv("Pakistan PSB EpiEstim Data.csv")
...
figure<-ggarrange(p,paklabs,pl2,pl3,pl4,pl5,
                  labels=c("PSB Incidence Plot",
                           "PSB Campaign Plot",
                           "PSB Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=10,scale=1)",
                           "PSB Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=10,SD=sqrt(10))",
                           "PSB Poliomyelitis Reproduction Number Estimation Plot Made with EpiFliter(shape=28,scale=1.5)",
                           "PSB Poliomyelitis Reproduction Number Estimation Plot Made with EpiEstim(mean=42,SD=3*sqrt(7))"
                  ),
