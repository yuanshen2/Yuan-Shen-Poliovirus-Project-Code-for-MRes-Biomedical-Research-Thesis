...
Pak<-ggplot(mydata, aes(x = Dates)) +
  ...
  theme_classic()+
  labs(x= "Dates", y = "Daily Case Incidence")
Pak

#Afghanistan Model Fitting
# Main packages
library("EpiEstim")
library("caTools")
# Main functions to run EpiFilter
...
library(ggplot2)
library(ggpubr)
figure<-ggarrange(Pak,Afg,Taj,Nig,
                  labels=c("A","B","C","D"),
                  ncol=2,nrow=2,
                  align="v")
